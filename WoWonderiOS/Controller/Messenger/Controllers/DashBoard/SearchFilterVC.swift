...
class SearchFilterVC: UIViewController,TTRangeSliderDelegate,SearchDelegate{
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.all.backgroundColor = .ButtonColor
        self.ageSwitch.thumbTintColor = .mainColor
        self.filterBtn.backgroundColor = .ButtonColor
    ...
    @IBAction func Gender(_ sender: UIButton) {
        if (sender.tag == 0){
            self.all.backgroundColor = UIColor.ButtonColor
            ...
            self.male.backgroundColor = .white
            self.female.backgroundColor = UIColor.ButtonColor
            ...
            self.all.backgroundColor = .white
            self.female.backgroundColor = .white
            self.male.backgroundColor = UIColor.ButtonColor
    ...
    @IBAction func Filter(_ sender: Any) {
        self.dismiss(animated: true) {
            self.delegate?.filterSearch(gender: self.gender ?? "", countryId: self.countryId ?? "", ageTo: self.age_to ?? "", ageFrom: self.age_from ?? "", verified: self.verify ?? "", status: self._status ?? "", profilePic: self.profilePicture.text ?? "")
        }
